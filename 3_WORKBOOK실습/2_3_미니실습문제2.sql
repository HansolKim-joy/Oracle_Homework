...
-- 2.춘 기술대학교의 교수 중 이름이 세 글자가 아닌 교수가 한 명 있다고 한다. 그 교수의 이름과 주민번호르르 화면에 출력하는 SQL 문장을 작성해보자.
--  (* 이때 올바르게 작성한 SQL문자의 결과 값이 예상과 다르게 나올 수 있다. 원인이 무엇일지 생각해볼 것)
SELECT PROFESSOR_NAME, PROFESSOR_SSN
FROM TB_PROFESSOR
WHERE PROFESSOR_NAME NOT LIKE '___';

-- 3. 춘 기술대학교의 남자 교수들의 이름과 나이를 출력하는 SQL 문장을 작성하시오. 단, 이때 나이가 적은 사람에서 많은 사람 순서로 화면에 출력되게 만드시오.
--  (단, 교수 중 2000년 이후 출생자는 없으며 출력 헤더는 "교수이름", "나이"로 한다. 나이는 '만'으로 계산한다.)
SELECT PROFESSOR_NAME 교수이름, EXTRACT(YEAR FROM SYSDATE) - TO_NUMBER(CONCAT(19, SUBSTR(PROFESSOR_SSN,1,2))) 나이
FROM TB_PROFESSOR
...
FROM DUAL;
-- 2049년 10월 11일을 의미
-- => YY는 모든 년도를 현재 세기(21세기) 기준으로 보여주어 2000년대로 시작하게 된다
...
FROM DUAL;
-- 2049년 10월 11일을 의미
-- => RR은 50년을 기준으로 50년보다 이상인 경우는 전 세기(20세기), 50년보다 미만인 경우는 현재 세기(21세기) 기준으로 보여준다.

-- 8. 춘 기술대학교의 2000 년도 이후 입학자들은 학번이 A로 시작하게 되어있다. 2000년도 이전 학번을 받은 학생들의 학번과 이름을 보여주는 SQL 문장을 작성하시오.
SELECT STUDENT_NO, STUDENT_NAME
FROM TB_STUDENT
WHERE STUDENT_NO NOT LIKE 'A%';

-- 9. 학번이 A517178인 한아름 학생의 학점 총 평균을 구하는 SQL문을 작성하시오. 
...
-- 단, 이떄 출력 화면의 헤더는 "년도", "년도 별 평점" 이라고 찍히게 하고, 점수는 반올림하여 소수점 이하 한 자리까지만 표시한다.
